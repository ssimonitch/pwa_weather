(function() {
  ...
  document.getElementById('butAddCity').addEventListener('click', function() {
    ...
    app.cacheCities(app.selectedCities);
  });
  ...
  app.cacheCities = function(selectedCities) {
  ...
  app.getCachedCities = function(cb) {
    localforage.getItem('selectedCities', function(err, cachedCities) {
      if (err) return cb(err);
      return cb(null, cachedCities || []);
    });
  };

  app.init = function() {
    // check for cached cities
    app.getCachedCities(function(err, cachedCities) {
      if (err) console.error(err);

      if (cachedCities.length === 0) {
        // if no cache, inject fake city
        app.updateForecastCard(injectedForecast);
        app.updateForecasts();
      } else {
        // otherwise set state and render card for each cached city
        cachedCities.forEach(data => {
